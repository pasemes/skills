...
**Fork addition — not upstream text.** One rule, carried at the project-style-guide layer named above, so where it meets a Google-layer rule — P1's one imperative action per step, above all — F1 wins.

- **F1 — Name the reader's situation.** The reader's situation survives into the sentence. A step may open on a lead-in clause before the imperative: "When you get stuck, ask the tutor:" rather than "Optional: ask the tutor a question." A section may open on the reader's state rather than on a bare list: "Three things are easy to discover only once they have already stopped you," above **Before you begin**. And where a rule's reason is one short clause, give it: "That refusal is structural rather than manners." F1 loosens the sentence, not the structure — the skeletons, the fact discipline and the severity ladder are unchanged. Severity Low; see [references/audit-checklist.md](references/audit-checklist.md).
