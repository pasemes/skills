...
Sixty rules in seven groups, plus one fork addition in an eighth. The `(inferred)` and `(convention)` labels mark rows that are not verbatim Google guidance; keep the label when you cite them. Source slugs are pages under `developers.google.com/style` unless another origin is named.
...
**F — fork addition (not upstream text)**

| ID | Rule | Severity | [EN] | Source | Owner file |
|----|------|----------|------|--------|-----------|
| F1 | Name the reader's situation: a lead-in clause may precede the imperative, a section may open on the reader's state, a short reason is given | Low | — | (fork addition — provenance in the repository README) | SKILL.md |
...
Structural and content rules apply unchanged: all of R, S1–S10 and S12, P1–P11, A1–A6, N1–N6, plus V1, V2, V3, V6, V7, V9, W1, W3, W6 and F1. A Japanese how-to still needs an audience statement, numbered steps, and verified flags.
