...
Regardless of mode, append an entry to `runs.log` in this skill directory. Create the file if it doesn't exist.
...
`runs.log` is this skill's only memory of its own runs. Anything that outlives a run — a workflow rule, a schema deviation, a stack gotcha — belongs to the consuming project, in the file it keeps its engineering notes in, and to the user's `config.json` where it changes this skill's behaviour.
